...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        #         image.append(self._video_idx_to_name[i])
        #     box = []
        #     for item in boxes_and_labels.keys():
        #         box.append(item)

            #res = list(set(image).difference(set(box)))
            #res.sort()
            #print(res)
        # if self._split == 'val':
        #     for i in range(len(self._image_paths)):
        #         print(self._image_paths[i])
        # image_paths = self._image_paths.keys()
        # res = list(set(image_paths).difference(set(box)))
        # res.sort()
        # print(res)
        #print("print split",len(self._image_paths))
        assert len(boxes_and_labels) == len(self._image_paths)
        # if self._split == 'val':
        #     #print(boxes_and_labels["00429"])
        #     print(boxes_and_labels["00430"])
